...
# Setup Plexus Archive Subgraph
# Usage: ./setup-subgraph.sh
...
echo -e "${BLUE}=== SETTING UP PLEXUS ARCHIVE SUBGRAPH ===${NC}"
...
# Check if the global 'graph' command is available (installed via pnpm's path)
if ! command -v graph &> /dev/null; then
    echo -e "${BLUE}Installing Graph CLI globally with pnpm...${NC}"
    
    # Attempt to install globally. This is where it will fail if `pnpm setup` hasn't run.
...
# Install dependencies locally using pnpm (This only works after CLI is installed above)
