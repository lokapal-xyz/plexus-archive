...
# Deploy PlexusArchive to Base networks
# Usage: ./deploy-archive.sh <network>
...
show_usage() {
    echo -e "${BLUE}Deploy PlexusArchive Script${NC}"
    echo ""
...
echo -e "${BLUE}=== DEPLOYING PLEXUS ARCHIVE TO ${NETWORK_NAME} ===${NC}"
...
DEPLOY_OUTPUT=$(forge script script/DeployPlexusArchive.s.sol:DeployPlexusArchive \
    --rpc-url "$RPC_URL" \
...
EOF
    ...
    if [ ! -z "$BASESCAN_API_KEY" ] && [ "$BASESCAN_API_KEY" != "your_basescan_api_key_here" ]; then
        ...
        forge verify-contract \
            ...
            "$CONTRACT_ADDRESS" \
            src/PlexusArchive.sol:PlexusArchive || {
    ...
    echo "1. Test: ./query-plexus.sh ${NETWORK} status"
    echo "2. Archive: ./archive-shard.sh ${NETWORK} \"Title\" \"Source\" \"Time 1\" \"Time 2\" \"Log\""
